...
import { getCustomFields, deleteCustomFieldById } from 'apiSdk/custom-fields';
...
function CustomFieldListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('custom_field', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/custom-fields/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('custom_field', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/custom-fields/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('custom_field', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/custom-fields/edit/${record.id}`} passHref legacyBehavior>
                          <Button
